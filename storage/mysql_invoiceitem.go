...
// used to work with mySQL - invoiceItem
type MySQLInvoiceItem struct {
	db *sql.DB
}

// returns a new pointer to MySQLInvoiceItem
func NewMySQLInvoiceItem(db *sql.DB) *MySQLInvoiceItem {
	return &MySQLInvoiceItem{db}
}

// implements the interface invoiceitem.Storage
...
func (p *MySQLInvoiceItem) CreateTx(tx *sql.Tx, headerID uint,
